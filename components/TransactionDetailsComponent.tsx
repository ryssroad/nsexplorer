...
> = ({ transaction, rawTransactionData }) => {
  ...
  useEffect(() => {
    async function fetchBlockHeight() {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_INDEXER_API_URL}/block/hash/${transaction.block_id}`);
        if (!response.ok) {
  ...
  return (
    <div className="p-5 space-y-5">
            ...
            <Table>
              <TableBody>
                ...
                <TableRow>
                  ...
                  <TableCell>{blockHeight ?? "Loading..."}</TableCell>
                  <TableCell className="text-slate-400">Status:</TableCell>
                  <TableCell className={status === 'Failed' ? 'text-red-600' : 'text-green-600'}>{status}</TableCell>
                </TableRow>
                <TableRow></TableRow>
              </TableBody>
            </Table>
      ...
      <div className="p-4 text-sm">
          <pre className="text-zinc-500 whitespace-pre-wrap break-all">{rawTransactionData}</pre>
        </div>
        </ScrollArea>
        </CardContent>
      </Card>
